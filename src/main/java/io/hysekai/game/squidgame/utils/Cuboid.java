...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable{
    ...
    public boolean isInside(Location location) {
        if (getWorld() == null)
            return false;
        return (location.getWorld().equals(getWorld()) && location.getBlockX() >= this.xmin && location.getBlockX() <= this.xmax && location.getBlockY() >= this.ymin && location.getBlockY() <= this.ymax && location.getBlockZ() >= this.zmin && location
                .getBlockZ() <= this.zmax);
    }
    ...
    public boolean hasLocationInside(Location loc){
        if(xmin <= loc.getX() && xmax >= loc.getX() && ymin <= loc.getY() && ymax >= loc.getY() && zmin <= loc.getZ() && zmax >= loc.getX() && w.getName().equals(loc.getWorld().getName())){
            return true;
        }
        else{
